<template>
    <article v-for="(title, index) in titles" class="w-[28em] h-full rounded-md mb-6" :key="title.id">
        <div class="flex bg-cyan-900 border-b border-cyan-400 rounded-t-md">
            <div class="shrink-0">
                <div class="flex">
        ...
        <div class="flex bg-cyan-900 py-2" :class="{'rounded-b-md': !open[index],}">
            <button
                class="flex items-center bg-white border border-cyan-400 rounded-md ring-2 ring-cyan-200 mx-3 px-3 py-2"
        ...
        <transition
            ...
            <div v-show="open[index]" class="flex border-l border-b border-r border-cyan-300 text-md leading-7 py-4 px-3">
                {{ title.summary }}
            </div>
        </transition>
    </article>
